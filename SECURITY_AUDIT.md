...
**Дата:** 2026-06-11 · **Версия:** 1.8.5 · **Метод:** 3 параллельных агента (main-процесс / рендерер+auth / конфиг+зависимости+секреты) + `npm audit` + поиск секретов по git-истории.
...
**Что мешает оценке A:** неподписанные авто-обновления (риск supply-chain), Forge-installer без checksum + допускается downgrade https→http (потенциальный RCE через MITM), Electron 33 EOL, нет CSP, `sandbox:false`. Перед широким распространением — закрыть HIGH.
