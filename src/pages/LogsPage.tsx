  ...
  return (
    ...
    >
      <div className="flex items-center justify-between">
        <h1 className="font-display text-4xl tracking-wide">ЛОГИ</h1>
      ...
      <div className="flex-1 overflow-auto rounded-2xl bg-black/40 p-4 font-mono text-[12px] leading-relaxed ring-1 ring-white/[0.06]">
        {lines.length === 0 ? (
          <div className="flex h-full items-center justify-center text-muted">
            Логов пока нет. Запусти игру через PLAY — вывод появится здесь.
          </div>
        ) : (
          <>
            {lines.map((l, i) => (
      ...
      <div className="flex items-center gap-2 text-xs text-muted">
        ...
        {lines.length} строк · скопируй и отправь в поддержку при вылете
      </div>
    </motion.div>
  );
}
